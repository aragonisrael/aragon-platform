...
const PROVISIONER_ROLES = new Set(['admin', 'instructor', 'temp_instructor']);
const MIGRATE_ROLES = new Set(['admin']);
...
async function requireCaller(req, allowedRoles) {
...
async function provisionOne(admin, user) {
...
async function provisionList(admin, users) {
  const results = [];
  for (const user of users || []) {
    results.push(await provisionOne(admin, user));
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (mode === 'one' || mode === 'provision_many') {
      const gate = await requireCaller(req, PROVISIONER_ROLES);
      ...
      if (mode === 'one') {
        ...
        const result = await provisionOne(admin, user);
      ...
      if (!userIds.length) return jsonResponse({ ok: false, error: 'userIds required' }, 400);
      ...
      return jsonResponse(await provisionList(admin, users));
    }

    if (mode === 'migrate_instructors' || mode === 'migrate_students') {
      const gate = await requireCaller(req, MIGRATE_ROLES);
      if (gate.error) return gate.error;
      const admin = gate.admin;

      const roles = mode === 'migrate_instructors'
        ? ['instructor', 'temp_instructor']
        : ['student'];

      // פעימות קטנות כדי לא לחתוך timeout — עד 40 בכל קריאה
      ...
      const payload = await provisionList(admin, users);
      return jsonResponse({
        ...payload,
